...
1. **Abra o Excel** e edite `Editor_Admin_Ingredientes_v2.xlsx` (ou sua cópia).
2. **Salvar como… → CSV (UTF-8)** e sobrescreva:
   ```
   admin_scaffold/admin/ingredients_master.csv
   ```
...
Rode:
```powershell
.tualizar_catalogo.bat
```

O que o `.bat` faz:
1. Executa `validate.py` sobre `ingredients_master.csv`.  
2. Executa `build_json.py` e gera `site/ingredients_master.json`.  
3. `git add . && git commit -m "Atualiza catálogo via planilha Excel" && git push origin main`.

**Se a validação falhar**, o `.bat` para e mostra o erro (não faz commit).

> Dica PowerShell: se aparecer “não reconhecido”, use `.tualizar_catalogo.bat`.  
...
- O arquivo `.github/workflows/publish-catalog.yml`:
  1) **Valida** o CSV  
  2) **Gera** o JSON em `site/ingredients_master.json`  
...
**Q:** _O `.bat` diz `KeyError: 'id'` (ou cabeçalho ausente)._  
...
> Dica: para rodar no PowerShell, use `.tualizar_catalogo.bat`.
...
**Pronto!** Este guia cobre o ciclo completo do **lado administrador**.  
Quando quiser, partimos para a **página do usuário (HTML/JS)** que consome este catálogo e permite selecionar ingredientes para montar a receita.
